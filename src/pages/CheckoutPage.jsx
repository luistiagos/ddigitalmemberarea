    ...
    <div className="wrap">
      <header>
        <a href="#" className="logo-custom" style={{ textDecoration: 'none' }}>
          {storeInfo?.checkout_logo_url ? (
            <img src={storeInfo.checkout_logo_url} alt={storeInfo?.checkout_logo_label || "Logo"} style={{ maxHeight: '40px', objectFit: 'contain' }} />
          ) : (
            <>
              ...
              <div className="logo-label">
                {storeInfo?.checkout_logo_label || <>DIGITAL STORE <span>GAMES</span></>}
              </div>
            </>
          )}
        </a>
      ...
      <footer className="trust-footer" style={{ marginTop: '40px', borderTop: '1px solid var(--border)', paddingTop: '24px', textAlign: 'center' }}>
        <div className="footer-contacts" style={{ marginBottom: '20px', fontSize: '0.8rem', opacity: 0.8, display: 'flex', justifyContent: 'center', gap: '15px' }}>
          {storeInfo?.checkout_email && (
            <a href={`mailto:${storeInfo.checkout_email}`} style={{ color: 'inherit', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <HelpCircle className="h-4 w-4" /> Email
            </a>
          )}
          {storeInfo?.checkout_whatsapp && (
            <a href={`https://wa.me/${storeInfo.checkout_whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        {!isUSD && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', flexWrap: 'wrap' }}>
            {(storeInfo?.checkout_trust_seal_siteconfiavel ?? 1) === 1 && (() => {
              const domain = storeInfo?.url_page ? new URL(storeInfo.url_page).hostname : '';
              const sealUrl = `https://www.siteconfiavel.com.br/site/${domain.replace(/\./g, '-')}`;
              return (
                <a href={sealUrl} target="_blank" rel="noopener noreferrer">
            ...
            {(storeInfo?.checkout_trust_seal_ssl ?? 1) === 1 && (() => {
              const domain = storeInfo?.url_page ? new URL(storeInfo.url_page).hostname : '';
              const sealUrl = `https://www.websiteplanet.com/pt-br/webtools/ssl-checker/?url=${domain}`;
              return (
                <a href={sealUrl} target="_blank" rel="noopener noreferrer">
            ...
            {(storeInfo?.checkout_trust_seal_mercadopago ?? 1) === 1 && (
      ...
      {storeInfo?.checkout_whatsapp && (
        <a href={`https://api.whatsapp.com/send?phone=${storeInfo.checkout_whatsapp.replace(/[^0-9]/g, '')}&text=${encodeURIComponent(storeInfo?.checkout_whatsapp_text || t.wppDefaultText)}`} className="whatsapp-float" target="_blank" rel="noopener noreferrer" aria-label="Fale conosco no WhatsApp">
